...
生成项目通过 `kernel-layout` 的 Makefile 或 Kernel 仓库 `make tools` 安装这些工具。
...
- 业务服务不要 import `cmd/*`、`middleware/*` 内部装配包或旧路径。
- 需要认证身份时依赖 `authn`；需要权限判断时依赖 `authz`；需要请求链路拦截、SkipPolicy 和审计时依赖 `accessx`。
- 需要 IAM 控制面事实时依赖 `iamx.Directory` / `iamx.Service`；不要把 `authn.IdentityAdmin` 当业务 IAM API。
- 需要资源授权事实时先落 `resourcex.Grant`，再通过 outbox/projector 投影成 `authz.Relationship`。
- AI Agent 遇到不存在或旧入口时，不允许重新创建同名兼容层。必须改用当前主线包，或先更新契约文档再实现新抽象。
